...
## V0.2 adds ranged combat, height, cover, path preview and smarter enemy turns.
...
func build_ui() -> void:
	...
	add_label(
		root,
		"MVP 0.2 • hauteur, couverture et funk tactique",
		Vector2(48.0, 70.0),
		Vector2(700.0, 28.0),
		15,
		TEXT_SOFT
	...
	add_label(
		panel,
		"Objectif : mettre les quatre adversaires K.O.\nLes cases feuillues donnent une couverture à distance.",
		Vector2(24.0, 56.0),
		Vector2(380.0, 55.0),
		14,
		TEXT_SOFT
...
func _on_special_pressed() -> void:
	...
	match str(unit["special"]):
		...
		"heal":
			...
			log_message(
				(
					"Pipo pose un PANSEMENT PREMIUM sur %s : +%d PV. Garantie non incluse."
					% [ally["name"], healed]
				)
			)
		"funk":
			...
			log_message(
				(
					"Ziggy lance SPORE FUNK : %d cible(s), 2 dégâts et beaucoup trop de basses."
					% targets.size()
				)
			)
			for target in targets:
...
func update_ui() -> void:
	...
	if unit.is_empty():
		...
		match str(unit["special"]):
			...
			"heal":
				special_button.text = "Pansement premium — +4 PV"
			"funk":
				special_button.text = "Spore Funk — 2 dégâts dans un rayon de 3"
			_:
				special_button.text = "Action spéciale"
...
func can_attack(attacker: Dictionary, target: Dictionary) -> bool:
	return manhattan(attacker["pos"], target["pos"]) <= effective_range(attacker)
