...
## V0.7 adds a lightweight campaign layer around the deterministic tactical battle.
## The player prepares a 3-hero squad, selects one module per hero, clears three missions,
## earns Spores XP and grows the squad between battles. Everything remains procedural.
...
# V0.7 campaign / preparation state.
...
func build_ui() -> void:
	...
	add_label(
		root,
		"MVP 0.7 • préparation d’escouade, modules, 3 missions et progression",
		Vector2(48.0, 70.0),
		Vector2(700.0, 28.0),
		15,
		TEXT_SOFT
...
func build_campaign_ui() -> void:
	...
	add_label(card, "Choisis exactement 3 héros. Chaque clic sur MODULE change leur spécialisation.", Vector2(40.0, 157.0), Vector2(950.0, 28.0), 14, TEXT_SOFT)
	...
	for index in range(hero_ids.size()):
		...
		var hero_button := add_button(card, "", Vector2(40.0, y), Vector2(280.0, 54.0))
		hero_button.pressed.connect(_on_hero_toggle.bind(hero_id))
		prep_hero_buttons[hero_id] = hero_button
		var module_button := add_button(card, "", Vector2(337.0, y), Vector2(655.0, 54.0))
	...
	debrief_card.position = Vector2(320.0, 175.0)
	debrief_card.size = Vector2(640.0, 410.0)
	debrief_card.add_theme_stylebox_override("panel", make_style(PANEL, 24, PANEL_EDGE))
	debrief_overlay.add_child(debrief_card)
	debrief_title_label = add_label(debrief_card, "", Vector2(34.0, 32.0), Vector2(570.0, 48.0), 29, GOLD)
	debrief_text_label = add_label(debrief_card, "", Vector2(34.0, 100.0), Vector2(570.0, 190.0), 16, TEXT)
	debrief_text_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	debrief_continue_button = add_button(debrief_card, "CONTINUER", Vector2(34.0, 318.0), Vector2(570.0, 54.0))
...
func update_prep_ui() -> void:
	...
	prep_status_label.text = "Escouade %d/3 • Spores XP %d • Rang %d\nBonus de rang : +%d PV max à tous les héros." % [count, campaign_spores, rank, rank - 1]
...
func _on_continue_campaign_pressed() -> void:
	...
	if mission_victory:
		...
		if campaign_mission_index >= 3:
			...
			campaign_wins = 0
			campaign_spores = 0
			prep_status_label.text = "Nouvelle tournée : la progression repart à zéro."
	show_prep_screen()
...
func show_debrief_screen() -> void:
	...
	if mission_victory:
		...
		debrief_text_label.text = "Spores XP gagnées : +%d\nBonus collectés : %d/%d\nSurvivants : %d/%d\n\nTotal : %d XP • Rang %d\nLe prochain combat applique automatiquement le bonus de rang." % [reward, bonus_collected, bonus_target_total, alive_count("player"), selected_hero_count(), campaign_spores, campaign_rank()]
		debrief_continue_button.text = "MISSION SUIVANTE" if campaign_mission_index < 2 else "TERMINER LA TOURNÉE"
	else:
		debrief_title_label.text = "MISSION ÉCHOUÉE"
		debrief_text_label.text = "Aucune Spores XP perdue.\nTu peux modifier l'escouade et les modules avant de retenter cette mission.\n\nTotal : %d XP • Rang %d" % [campaign_spores, campaign_rank()]
...
func update_ui() -> void:
	...
	if unit.is_empty():
		...
		match str(unit["special"]):
			"hat":
				special_button.text = "Coup de chapeau — 4 dégâts + poussée"
			"heal":
				special_button.text = "Sauvetage élastique — soin + Garde + traction"
			"funk":
				special_button.text = "Annuler Spore Funk" if special_targeting else "Spore Funk — cibler une zone"
			"flare":
				special_button.text = "Tir prismatique — cible auto, couvert ignoré"
			_:
				special_button.text = "Action spéciale"
