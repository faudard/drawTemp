...
## V0.5 deepens tactical interactions with reactions, forced movement and targeted areas.
## It also adds optional vinyl pickups while preserving the retrieve-and-extract mission.
...
func build_ui() -> void:
	...
	add_label(
		root,
		"MVP 0.5 • réactions, poussée/traction, zones télégraphiées et objectifs bonus",
		Vector2(48.0, 70.0),
		Vector2(700.0, 28.0),
		15,
		TEXT_SOFT
	...
	add_label(
		...
		(
			...
			+ "▲ hauteur • ♛ objectif • ◉ vinyle bonus • spores = 1 dégât • clic droit = orientation / annuler ciblage"
		),
		Vector2(48.0, 711.0),
		Vector2(730.0, 43.0),
		13,
...
func force_move(target: Dictionary, direction: Vector2i, distance: int, reason: String) -> void:
	for _step in range(distance):
		var next_cell: Vector2i = target["pos"] + direction
