...
## V0.8 turns the campaign into a small build-crafting loop.
## Heroes now spend Focus on two active skills with cooldowns, enemies have archetype skills,
## the Archivist boss changes phase, and mission rewards unlock persistent equipment.
...
# V0.8 campaign / preparation / equipment state.
...
# V0.8 active skills / equipment / reward layer.
...
func build_ui() -> void:
	...
	add_label(
		root,
		"MVP 0.8 • Focus, cooldowns, archétypes ennemis, boss en phases et équipement",
		Vector2(48.0, 70.0),
		Vector2(700.0, 28.0),
		15,
		TEXT_SOFT
...
func build_campaign_ui() -> void:
	...
	var card = Panel.new()
	card.position = Vector2(120.0, 70.0)
	card.size = Vector2(1040.0, 620.0)
	...
	prep_status_label = add_label(card, "", Vector2(40.0, 511.0), Vector2(620.0, 58.0), 14, TEXT)
	prep_status_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	prep_start_button = add_button(card, "LANCER LA MISSION", Vector2(690.0, 519.0), Vector2(302.0, 52.0))
...
func update_prep_ui() -> void:
	...
	prep_status_label.text = "Escouade %d/3 • Spores XP %d • Rang %d • Reliques %d\nBonus de rang : +%d PV max à tous les héros." % [count, campaign_spores, rank, unlocked_equipment.size(), rank - 1]
...
func _on_reward_selected(index: int) -> void:
	...
	if not unlocked_equipment.has(gear_id):
		unlocked_equipment.append(gear_id)
	reward_selected = true
	debrief_reward_label.text = "RÉCOMPENSE CHOISIE : %s" % equipment_name(gear_id)
...
func _on_continue_campaign_pressed() -> void:
	...
	if mission_victory:
		campaign_wins += 1
		campaign_mission_index += 1
		if campaign_mission_index >= 3:
			campaign_completed = true
			campaign_mission_index = 0
			campaign_wins = 0
			prep_status_label.text = "NOUVELLE TOURNÉE+ : XP et équipements sont conservés."
	show_prep_screen()
...
func show_debrief_screen() -> void:
	...
	if mission_victory:
		var reward = 3 + bonus_collected + alive_count("player")
		if not mission_reward_granted:
			campaign_spores += reward
			mission_reward_granted = true
		debrief_title_label.text = "MISSION RÉUSSIE"
		debrief_text_label.text = "Spores XP gagnées : +%d • Total %d • Rang %d\nBonus : %d/%d • Survivants : %d/%d\n\nChoisis un équipement permanent pour la Serre tactique." % [reward, campaign_spores, campaign_rank(), bonus_collected, bonus_target_total, alive_count("player"), selected_hero_count()]
...
func reset_battle() -> void:
	...
	log_message("Déploiement : %s. Rang d'escouade %d." % [mission["name"], campaign_rank()])
...
func objective_status_text() -> String:
	...
	if mission_objective == "crown":
		if crown_carrier_id.is_empty():
			return "OBJECTIF : atteindre ♛ (%d,%d) • Bonus %d/%d • Équipe %d/%d • Ennemis %d/%d" % [crown_cell.x + 1, crown_cell.y + 1, bonus_collected, bonus_target_total, alive_count("player"), squad_total, alive_count("enemy"), enemies_total_initial]
		var carrier = unit_by_id(crown_carrier_id)
		return "COURONNE : %s • sortie verte • Bonus %d/%d • Équipe %d/%d • Ennemis %d/%d" % [carrier["name"], bonus_collected, bonus_target_total, alive_count("player"), squad_total, alive_count("enemy"), enemies_total_initial]
	if mission_objective == "survive":
		return "TENIR : manche %d/%d • Bonus %d/%d • Équipe %d/%d • Ennemis %d/%d" % [current_turn, survival_rounds, bonus_collected, bonus_target_total, alive_count("player"), squad_total, alive_count("enemy"), enemies_total_initial]
	return "ÉLIMINATION : Bonus %d/%d • Équipe %d/%d • Ennemis %d/%d • Rang %d" % [bonus_collected, bonus_target_total, alive_count("player"), squad_total, alive_count("enemy"), enemies_total_initial, campaign_rank()]
