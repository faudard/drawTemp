...
## V0.4 turns initiative into a real alternating activation timeline.
## It also adds a retrieve-and-extract objective, dangerous terrain and side/back attacks.
...
func build_ui() -> void:
	...
	add_label(
		root,
		"MVP 0.4 • timeline alternée, mission d'extraction et terrain dangereux",
		Vector2(48.0, 70.0),
		Vector2(700.0, 28.0),
		15,
		TEXT_SOFT
	...
	timeline_label = add_label(panel, "", Vector2(24.0, 151.0), Vector2(380.0, 43.0), 13, TEXT_SOFT)
	timeline_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	selected_label = add_label(panel, "", Vector2(24.0, 197.0), Vector2(380.0, 77.0), 14, TEXT)
	preview_label = add_label(panel, "", Vector2(24.0, 274.0), Vector2(380.0, 50.0), 13, GOLD)
	preview_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART

	special_button = add_button(
		panel, "Action spéciale", Vector2(24.0, 329.0), Vector2(382.0, 44.0)
	)
	special_button.pressed.connect(_on_special_pressed)

	end_turn_button = add_button(panel, "Terminer l'activation", Vector2(24.0, 379.0), Vector2(382.0, 44.0))
	end_turn_button.pressed.connect(_on_end_turn_pressed)

	var restart_button := add_button(
		panel, "Recommencer la mission", Vector2(24.0, 429.0), Vector2(382.0, 39.0)
	)
	restart_button.pressed.connect(reset_battle)

	add_label(
		panel, "JOURNAL DU NARRATEUR", Vector2(24.0, 478.0), Vector2(382.0, 24.0), 12, TEXT_SOFT
	)
	log_label = add_label(panel, "", Vector2(24.0, 503.0), Vector2(382.0, 84.0), 12, TEXT)
	log_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART

	add_label(
		root,
		"L'unité active est imposée par la timeline. Marche et action peuvent être faites dans l'ordre voulu.\n▲ hauteur • ♛ objectif • flaques violettes = 1 dégât en fin d'activation • clic droit = orientation finale",
		Vector2(48.0, 711.0),
		Vector2(730.0, 43.0),
		13,
		TEXT_SOFT
...
func check_mission_end() -> bool:
	...
	if not crown_carrier_id.is_empty():
		...
		if not carrier.is_empty() and int(carrier["hp"]) > 0 and extraction_cells.has(carrier["pos"]):
			...
			selected_id = ""
			active_unit_id = ""
			log_message("VICTOIRE ! La Couronne Beatbox est extraite. Elle exige déjà une loge individuelle.")
			refresh_all()
			return true
...
func update_ui() -> void:
	...
	timeline_label.text = "TIMELINE : " + timeline_summary()
	var unit := selected_unit()
	if unit.is_empty():
		...
		match str(unit["special"]):
			"hat": special_button.text = "Coup de chapeau — 5 dégâts au contact"
			"heal": special_button.text = "Pansement premium — +4 PV + Garde"
			"funk": special_button.text = "Spore Funk — 2 dégâts + Ralenti rayon 3"
...
func objective_status_text() -> String:
	...
	if crown_carrier_id.is_empty():
		return "OBJECTIF : atteindre ♛ en (%d,%d) • Équipe %d/3 • Ennemis %d/4" % [crown_cell.x + 1, crown_cell.y + 1, alive_count("player"), alive_count("enemy")]
	var carrier := unit_by_id(crown_carrier_id)
	return "COURONNE : %s la porte • rejoins une case verte • Équipe %d/3 • Ennemis %d/4" % [carrier["name"], alive_count("player"), alive_count("enemy")]
