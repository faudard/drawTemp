...
func present(text_value: String, color: Color, kind: String = "default") -> void:
	...
	tween.tween_property(scale, "x", 1.0, 0.14)
	tween.tween_property(scale, "y", 1.0, 0.14)
	tween.tween_property(scale, "z", 1.0, 0.14)
	tween.tween_property(self, "position:y", position.y + 0.46, 0.52)
	tween.set_parallel(false)
	tween.tween_interval(0.14)
	tween.set_parallel(true)
	tween.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	tween.tween_property(label, "modulate:a", 0.0, 0.20)
	tween.tween_property(self, "position:y", position.y + 0.18, 0.20)
