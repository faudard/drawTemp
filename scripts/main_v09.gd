...
## V0.9 turns the prototype into a persistent mini-campaign.
## Progress, squad loadout, difficulty, mission unlocks and relics are saved to user://.
## Missions can be replayed, optional contracts award bonus XP, and intermission events alter the next battle.
...
# Campaign / preparation / equipment state inherited from V0.8.
var campaign_mode = "prep"
var campaign_mission_index = 0
var campaign_spores = 0
var campaign_wins = 0
...
func build_ui() -> void:
	...
	add_label(
		root,
		"MVP 0.9 • campagne persistante, contrats, difficulté et événements intermission",
		Vector2(48.0, 70.0),
		Vector2(700.0, 28.0),
		15,
		TEXT_SOFT
...
func build_campaign_ui() -> void:
	...
	add_label(card, "Choisis 3 héros, un MODULE et un ÉQUIPEMENT débloqué par héros.", Vector2(40.0, 157.0), Vector2(950.0, 28.0), 14, TEXT_SOFT)
	...
	for index in range(hero_ids.size()):
		...
		var hero_button = add_button(card, "", Vector2(40.0, y), Vector2(245.0, 54.0))
		hero_button.pressed.connect(_on_hero_toggle.bind(hero_id))
		prep_hero_buttons[hero_id] = hero_button
		var module_button = add_button(card, "", Vector2(298.0, y), Vector2(346.0, 54.0))
		module_button.pressed.connect(_on_module_cycle.bind(hero_id))
		prep_module_buttons[hero_id] = module_button
		var gear_button = add_button(card, "", Vector2(657.0, y), Vector2(335.0, 54.0))
...
func update_prep_ui() -> void:
	...
	var mission = mission_meta(campaign_mission_index)
	prep_mission_label.text = "MISSION %d/3 — %s\n%s" % [campaign_mission_index + 1, mission["name"], mission["brief"]]
	for hero_id in prep_hero_buttons.keys():
		var button: Button = prep_hero_buttons[hero_id]
		var selected = bool(hero_selected.get(hero_id, false))
		button.text = "%s  %s\n%s" % ["✓" if selected else "○", hero_name(hero_id), hero_role(hero_id)]
		var module_button: Button = prep_module_buttons[hero_id]
		module_button.text = "MODULE : %s\n%s" % [hero_module_name(hero_id), hero_module_description(hero_id)]
		var gear_button: Button = prep_gear_buttons[hero_id]
		var gear_id = str(hero_equipment.get(hero_id, "none"))
		gear_button.text = "ÉQUIPEMENT : %s\n%s" % [equipment_name(gear_id), equipment_description(gear_id)]
	var count = selected_hero_count()
	var rank = campaign_rank()
	prep_status_label.text = "Escouade %d/3 • Spores XP %d • Rang %d • Tournée %d • Reliques %d\nBonus de rang : +%d PV max • Prochain bonus : %s" % [count, campaign_spores, rank, campaign_loop + 1, unlocked_equipment.size(), rank - 1, mission_buff_name(next_mission_buff)]
	prep_mission_button.text = "MISSION : %d/3 — %s  [débloquées %d/3]" % [campaign_mission_index + 1, mission["name"], campaign_max_unlocked + 1]
	prep_difficulty_button.text = "DIFFICULTÉ : %s — %s" % [difficulty_name(), difficulty_description()]
	prep_save_label.text = "Sauvegarde auto : %s • Contrat : %s" % [SAVE_PATH, secondary_objective_text(campaign_mission_index)]
...
func _on_hero_toggle(hero_id: String) -> void:
	var selected = bool(hero_selected.get(hero_id, false))
	if selected:
		hero_selected[hero_id] = false
	elif selected_hero_count() < 3:
		hero_selected[hero_id] = true
	else:
...
func _on_module_cycle(hero_id: String) -> void:
	hero_modules[hero_id] = (int(hero_modules.get(hero_id, 0)) + 1) % 2
	save_campaign()
	update_prep_ui()

func _on_gear_cycle(hero_id: String) -> void:
	var choices: Array = ["none"]
	for equipment_id in unlocked_equipment:
		choices.append(equipment_id)
	var current = str(hero_equipment.get(hero_id, "none"))
	var index = choices.find(current)
	if index < 0:
		index = 0
	hero_equipment[hero_id] = choices[(index + 1) % choices.size()]
	save_campaign()
	update_prep_ui()
...
func equipment_name(equipment_id: String) -> String:
	match equipment_id:
		"sprout_badge":
			return "Badge pousse"
		"rhythm_boots":
			return "Bottes rythmiques"
		"echo_lens":
			return "Lentille écho"
		"thorn_ring":
			return "Anneau d'épine"
		"hot_capacitor":
			return "Condensateur chaud"
		"moss_charm":
			return "Charme mousse"
		"crown_core":
			return "Cœur de couronne"
		"mycelium_plate":
			return "Plaque mycélienne"
		"phase_compass":
			return "Compas de phase"
		_:
			return "Aucun"
...
func equipment_description(equipment_id: String) -> String:
	match equipment_id:
		...
		"moss_charm":
			return "+1 INIT"
		"crown_core":
			return "+1 ATQ et +1 Focus max"
		"mycelium_plate":
			return "+2 PV et GARDE au départ"
		"phase_compass":
			return "+1 portée à distance et +1 INIT"
		_:
			return "Pas de bonus"
...
func _on_reward_selected(index: int) -> void:
	...
	var duplicate = unlocked_equipment.has(gear_id)
	if duplicate:
		campaign_spores += 2
	else:
		unlocked_equipment.append(gear_id)
	reward_selected = true
	debrief_reward_label.text = (
		"RELIQUE DÉJÀ CONNUE : +2 Spores XP • Total %d • Rang %d" % [campaign_spores, campaign_rank()]
		if duplicate
...
func _on_continue_campaign_pressed() -> void:
	debrief_overlay.visible = false
	if mission_victory and campaign_mission_index < 2:
		show_intermission_event(campaign_mission_index)
		return
	if mission_victory:
...
func show_debrief_screen() -> void:
	if campaign_mode != "battle":
		return
	campaign_mode = "debrief"
	...
	if mission_victory:
		var contract_complete = secondary_objective_completed()
		var contract_reward = 2 if contract_complete else 0
		var reward = max(1, 3 + bonus_collected + alive_count("player") + difficulty_xp_bonus() + contract_reward)
		if not mission_reward_granted:
			campaign_spores += reward
			mission_reward_granted = true
			campaign_max_unlocked = max(campaign_max_unlocked, min(2, campaign_mission_index + 1))
			next_mission_buff = ""
		debrief_title_label.text = "MISSION RÉUSSIE"
		debrief_text_label.text = "Spores XP : +%d • Total %d • Rang %d\nContrat : %s %s • difficulté %s (%s)\nBonus : %d/%d • Survivants : %d/%d\n\nChoisis un équipement permanent pour la Serre tactique." % [reward, campaign_spores, campaign_rank(), "RÉUSSI" if contract_complete else "MANQUÉ", "+2 XP" if contract_complete else "+0 XP", difficulty_name(), difficulty_xp_label(), bonus_collected, bonus_target_total, alive_count("player"), selected_hero_count()]
		reward_options = reward_pool(campaign_mission_index)
		...
		for i in range(debrief_reward_buttons.size()):
			...
			var button: Button = debrief_reward_buttons[i]
			button.text = "%s — %s" % [equipment_name(gear_id), equipment_description(gear_id)]
		debrief_continue_button.disabled = true
		debrief_continue_button.text = "MISSION SUIVANTE" if campaign_mission_index < 2 else "NOUVELLE TOURNÉE+"
	else:
		debrief_title_label.text = "MISSION ÉCHOUÉE"
		debrief_text_label.text = "Aucune Spores XP perdue.\nTu peux modifier escouade, modules et équipement avant de retenter.\n\nTotal : %d XP • Rang %d" % [campaign_spores, campaign_rank()]
		debrief_reward_label.text = ""
...
func _on_reset_save_pressed() -> void:
	if not reset_save_armed:
		...
		prep_status_label.text = "Un second clic efface la progression V0.9 et recrée une sauvegarde neuve."
		return
	reset_save_armed = false
	campaign_mission_index = 0
	campaign_max_unlocked = 0
	campaign_loop = 0
	campaign_spores = 0
	campaign_wins = 0
...
func _on_mission_cycle_pressed() -> void:
	reset_save_armed = false
	prep_reset_button.text = "NOUVELLE CAMPAGNE"
	var available = clamp(campaign_max_unlocked, 0, 2)
	campaign_mission_index = (campaign_mission_index + 1) % (available + 1)
	save_campaign()
	update_prep_ui()
...
func apply_difficulty_to_enemies() -> void:
	for unit in units:
		if unit["team"] != "enemy":
			continue
		if campaign_loop > 0:
			unit["hp"] = int(unit["hp"]) + campaign_loop
			unit["max_hp"] = int(unit["max_hp"]) + campaign_loop
			unit["attack"] = int(unit["attack"]) + int(campaign_loop / 2)
...
func apply_pending_mission_buff() -> void:
	if next_mission_buff.is_empty():
		return
	for unit in units:
		if unit["team"] != "player":
			continue
		if next_mission_buff == "focus":
			unit["max_focus"] = int(unit.get("max_focus", 2)) + 1
			unit["focus"] = int(unit["max_focus"])
		elif next_mission_buff == "guard":
			add_status(unit, "guarded")
	log_message("BONUS D'INTERMISSION : %s." % mission_buff_name(next_mission_buff))


func secondary_objective_text(index: int) -> String:
	match index:
		0:
			return "Collection complète — récupérer les 2 vinyles"
		1:
			return "Standing ovation — terminer avec les 3 héros debout"
		_:
			return "Rappel express — gagner avant la fin de la manche 4"


func secondary_objective_progress_text() -> String:
	match campaign_mission_index:
		0:
...
func show_intermission_event(source_mission: int) -> void:
	campaign_mode = "event"
	event_source_mission = source_mission
	prep_overlay.visible = false
	debrief_overlay.visible = false
	event_overlay.visible = true
	if source_mission == 0:
		event_title_label.text = "INTERMISSION — JAM DES LUCIOLES"
		event_text_label.text = "À la sortie du jardin, une bande de lucioles propose un bœuf clandestin.\nTu peux rester pour récolter de l'expérience, ou récupérer leur condensateur pour le prochain combat."
		event_choice_a.text = "RESTER POUR LE JAM — +2 Spores XP"
		event_choice_b.text = "PRENDRE LE CONDENSATEUR — +1 Focus max au prochain combat"
	else:
		event_title_label.text = "INTERMISSION — ATELIER DE MOUSSE"
		event_text_label.text = "Le vieux Myco-Mécano veut payer la troupe pour son concert.\nDes spores sonnantes tout de suite, ou une couche de mousse protectrice pour le prochain déploiement ?"
		event_choice_a.text = "PRENDRE LE CACHET — +2 Spores XP"
		event_choice_b.text = "RENFORCER LES CHAPEAUX — GARDE au prochain combat"


func _on_event_choice(choice: int) -> void:
	if event_source_mission < 0:
		return
	if choice == 0:
		campaign_spores += 2
	else:
		next_mission_buff = "focus" if event_source_mission == 0 else "guard"
	event_overlay.visible = false
...
func progress_after_victory() -> void:
	campaign_wins += 1
	var completed_index = campaign_mission_index
	campaign_max_unlocked = max(campaign_max_unlocked, min(2, completed_index + 1))
	if completed_index >= 2:
		...
		campaign_mission_index = min(completed_index + 1, campaign_max_unlocked)
	save_campaign()
	show_prep_screen()
...
func save_campaign() -> void:
	var data = {
		...
		"campaign_mission_index": campaign_mission_index,
		"campaign_max_unlocked": campaign_max_unlocked,
		"campaign_loop": campaign_loop,
		"campaign_spores": campaign_spores,
...
func load_campaign_save() -> void:
	...
	campaign_mission_index = clamp(int(data.get("campaign_mission_index", 0)), 0, 2)
	campaign_max_unlocked = clamp(int(data.get("campaign_max_unlocked", campaign_mission_index)), 0, 2)
	campaign_loop = max(0, int(data.get("campaign_loop", 0)))
	campaign_spores = max(0, int(data.get("campaign_spores", 0)))
	campaign_wins = max(0, int(data.get("campaign_wins", 0)))
	...
	if typeof(saved_selected) == TYPE_DICTIONARY:
		var selected_data: Dictionary = saved_selected
		for hero_id in hero_selected.keys():
			if selected_data.has(hero_id):
				hero_selected[hero_id] = bool(selected_data[hero_id])
	var saved_modules: Variant = data.get("hero_modules", {})
	if typeof(saved_modules) == TYPE_DICTIONARY:
		var modules_data: Dictionary = saved_modules
		for hero_id in hero_modules.keys():
			if modules_data.has(hero_id):
				hero_modules[hero_id] = clamp(int(modules_data[hero_id]), 0, 1)
	var saved_gear: Variant = data.get("hero_equipment", {})
	if typeof(saved_gear) == TYPE_DICTIONARY:
		var gear_data: Dictionary = saved_gear
		for hero_id in hero_equipment.keys():
			if gear_data.has(hero_id):
				hero_equipment[hero_id] = str(gear_data[hero_id])
	var saved_unlocks: Variant = data.get("unlocked_equipment", [])
	if typeof(saved_unlocks) == TYPE_ARRAY:
		var unlock_data: Array = saved_unlocks
		unlocked_equipment.clear()
		for equipment_id in unlock_data:
			var id = str(equipment_id)
			if not unlocked_equipment.has(id):
				unlocked_equipment.append(id)
	for hero_id in hero_equipment.keys():
		var equipped = str(hero_equipment[hero_id])
		if equipped != "none" and not unlocked_equipment.has(equipped):
			hero_equipment[hero_id] = "none"
	campaign_max_unlocked = max(campaign_max_unlocked, campaign_mission_index)
...
func campaign_rank() -> int:
	return 1 + int(campaign_spores / 6)
...
func hero_role(hero_id: String) -> String:
	match hero_id:
		"momo":
			return "Brave • mêlée"
		"pipo":
			return "Soutien • mêlée"
		"ziggy":
			return "Sporeur funky • portée 3"
		"luma":
			return "Éclaireuse prismatique • portée 3"
		_:
			return "Héros"


func hero_module_name(hero_id: String) -> String:
	var index = int(hero_modules.get(hero_id, 0))
	match hero_id:
		"momo":
...
func mission_meta(index: int) -> Dictionary:
	match index:
		0:
			return {"name": "La Couronne Beatbox", "brief": "Infiltre le jardin, récupère ♛ puis ramène-la dans la zone verte. Les vinyles restent optionnels."}
		1:
			return {"name": "Tenir la Scène", "brief": "Survis 3 manches au milieu des spores, ou mets toute la bande adverse K.O. avant la fin."}
		_:
			return {"name": "Dernier Rappel", "brief": "Élimine la troupe d'élite. Le terrain est plus vertical et les angles de tir comptent davantage."}
...
func reset_battle() -> void:
	...
	setup_mission_environment(campaign_mission_index)
	append_selected_squad(campaign_mission_index)
	apply_pending_mission_buff()
	append_mission_enemies(campaign_mission_index)
	apply_difficulty_to_enemies()
	enemies_total_initial = alive_count("enemy")
	bonus_target_total = bonus_cells.size()
	selected_id = ""
	var mission = mission_meta(campaign_mission_index)
	mission_description_label.text = "MISSION %d/3 : %s\n%s" % [campaign_mission_index + 1, mission["name"], mission["brief"]]
	log_message("Déploiement : %s. Rang %d • %s." % [mission["name"], campaign_rank(), difficulty_name()])
	log_message("CONTRAT : %s" % secondary_objective_text(campaign_mission_index))
	if campaign_mission_index == 0:
		log_message("Récupère la Couronne Beatbox puis reviens dans la zone verte.")
	elif campaign_mission_index == 1:
...
func setup_mission_environment(index: int) -> void:
	...
	match index:
		0:
			mission_objective = "crown"
			obstacles = [Vector2i(4, 2), Vector2i(5, 2), Vector2i(4, 5), Vector2i(5, 5), Vector2i(7, 3), Vector2i(2, 3)]
			...
			set_height([Vector2i(1, 1), Vector2i(2, 1), Vector2i(3, 5), Vector2i(3, 6), Vector2i(6, 1), Vector2i(6, 2), Vector2i(8, 4)], 1)
			set_height([Vector2i(8, 5)], 2)
		1:
			mission_objective = "survive"
			survival_rounds = 3
			obstacles = [Vector2i(2, 2), Vector2i(7, 2), Vector2i(2, 5), Vector2i(7, 5), Vector2i(4, 3), Vector2i(5, 4)]
			cover_cells = [Vector2i(3, 2), Vector2i(6, 2), Vector2i(3, 5), Vector2i(6, 5)]
			hazard_cells = [Vector2i(0, 1), Vector2i(1, 1), Vector2i(8, 6), Vector2i(9, 6), Vector2i(4, 1), Vector2i(6, 6)]
			bonus_cells = [Vector2i(0, 7), Vector2i(9, 0)]
			set_height([Vector2i(3, 3), Vector2i(6, 4), Vector2i(4, 6), Vector2i(5, 1)], 1)
			set_height([Vector2i(4, 0), Vector2i(5, 7)], 2)
		_:
			mission_objective = "eliminate"
			obstacles = [Vector2i(3, 1), Vector2i(6, 1), Vector2i(3, 6), Vector2i(6, 6), Vector2i(4, 3), Vector2i(5, 3), Vector2i(4, 4), Vector2i(5, 4)]
			cover_cells = [Vector2i(1, 3), Vector2i(2, 5), Vector2i(7, 2), Vector2i(8, 4), Vector2i(5, 6)]
			hazard_cells = [Vector2i(2, 1), Vector2i(7, 1), Vector2i(2, 6), Vector2i(7, 6), Vector2i(0, 4), Vector2i(9, 3)]
			bonus_cells = [Vector2i(1, 0), Vector2i(8, 7)]
			set_height([Vector2i(1, 2), Vector2i(2, 2), Vector2i(7, 5), Vector2i(8, 5), Vector2i(4, 1), Vector2i(6, 6)], 1)
			set_height([Vector2i(0, 0), Vector2i(9, 7), Vector2i(5, 0)], 2)


func append_selected_squad(index: int) -> void:
	var starts: Array
	if index == 0:
		starts = [Vector2i(1, 6), Vector2i(2, 7), Vector2i(0, 6)]
	elif index == 1:
		starts = [Vector2i(4, 6), Vector2i(5, 6), Vector2i(4, 7)]
	else:
		starts = [Vector2i(0, 7), Vector2i(1, 7), Vector2i(0, 6)]
	var selected_ids: Array = []
	for hero_id in ["momo", "pipo", "ziggy", "luma"]:
		if bool(hero_selected.get(hero_id, false)):
			selected_ids.append(hero_id)
	for i in range(selected_ids.size()):
		units.append(make_hero(str(selected_ids[i]), starts[i]))
...
func make_hero(hero_id: String, position: Vector2i) -> Dictionary:
	...
	match hero_id:
		"momo":
			unit = make_unit("momo", "Momo", "Brave", "player", position, 13, 3, 4, 1, Color("#f27b72"), "hat")
		"pipo":
			unit = make_unit("pipo", "Pipo", "Soutien", "player", position, 10, 2, 4, 1, Color("#6cc5c9"), "heal")
		"ziggy":
			unit = make_unit("ziggy", "Ziggy", "Sporeur funky", "player", position, 8, 2, 3, 3, Color("#c27be0"), "funk")
		_:
			unit = make_unit("luma", "Luma", "Éclaireuse prismatique", "player", position, 9, 2, 4, 3, Color("#77b7ff"), "flare")
	var rank_bonus = campaign_rank() - 1
	unit["hp"] = int(unit["hp"]) + rank_bonus
	unit["max_hp"] = int(unit["max_hp"]) + rank_bonus
	var module_index = int(hero_modules.get(hero_id, 0))
	if hero_id == "momo":
		if module_index == 0:
			unit["attack"] = int(unit["attack"]) + 1
		else:
			unit["move"] = int(unit["move"]) + 1
	elif hero_id == "ziggy" and module_index == 1:
		unit["move"] = int(unit["move"]) + 1
	elif hero_id == "luma":
		if module_index == 0:
			unit["range"] = int(unit["range"]) + 1
		else:
			unit["attack"] = int(unit["attack"]) + 1
	apply_equipment_to_hero(unit, str(hero_equipment.get(hero_id, "none")))
	unit["focus"] = int(unit["max_focus"])
	return unit
...
func apply_equipment_to_hero(unit: Dictionary, equipment_id: String) -> void:
	match equipment_id:
		...
		"thorn_ring":
			unit["attack"] = int(unit["attack"]) + 1
		"hot_capacitor":
			unit["max_focus"] = int(unit["max_focus"]) + 1
		"moss_charm":
			unit["initiative"] = int(unit["initiative"]) + 1
		"crown_core":
			unit["attack"] = int(unit["attack"]) + 1
			unit["max_focus"] = int(unit["max_focus"]) + 1
		"mycelium_plate":
...
func append_mission_enemies(index: int) -> void:
	if index == 0:
		units.append(make_unit("grincheux", "Grincheux", "Garde", "enemy", Vector2i(8, 1), 8, 2, 3, 1, Color("#9a7be0"), ""))
		units.append(make_unit("baveux", "Baveux", "Gluant", "enemy", Vector2i(8, 6), 7, 2, 3, 1, Color("#f3a65a"), ""))
		units.append(make_unit("comptable", "Le Comptable", "Archer fiscal", "enemy", Vector2i(6, 0), 10, 2, 2, 3, Color("#e2cf63"), ""))
		units.append(make_unit("dj_morille", "DJ Morille", "Disc-jockey hostile", "enemy", Vector2i(9, 4), 7, 2, 3, 2, Color("#db6f91"), ""))
	elif index == 1:
		units.append(make_unit("baveux", "Baveux", "Gluant", "enemy", Vector2i(1, 1), 8, 2, 4, 1, Color("#f3a65a"), ""))
		units.append(make_unit("grincheux", "Grincheux", "Garde", "enemy", Vector2i(8, 1), 9, 2, 3, 1, Color("#9a7be0"), ""))
		units.append(make_unit("comptable", "Le Comptable", "Archer fiscal", "enemy", Vector2i(9, 4), 10, 2, 2, 3, Color("#e2cf63"), ""))
		units.append(make_unit("choriste", "Choriste Moisi", "Harceleur", "enemy", Vector2i(0, 4), 7, 2, 4, 2, Color("#72c987"), ""))
		units.append(make_unit("dj_morille", "DJ Morille", "Disc-jockey hostile", "enemy", Vector2i(5, 0), 8, 2, 3, 2, Color("#db6f91"), ""))
	else:
		units.append(make_unit("grincheux", "Grincheux", "Garde d'élite", "enemy", Vector2i(8, 0), 11, 3, 3, 1, Color("#9a7be0"), ""))
		units.append(make_unit("baveux", "Baveux", "Gluant vétéran", "enemy", Vector2i(9, 2), 9, 2, 4, 1, Color("#f3a65a"), ""))
		units.append(make_unit("comptable", "Le Comptable", "Archer fiscal senior", "enemy", Vector2i(5, 0), 11, 3, 2, 4, Color("#e2cf63"), ""))
		units.append(make_unit("dj_morille", "DJ Morille", "Chef de piste", "enemy", Vector2i(8, 5), 10, 3, 3, 2, Color("#db6f91"), ""))
		units.append(make_unit("archiviste", "L'Archiviste", "Boss mycélien", "enemy", Vector2i(5, 7), 15, 3, 3, 2, Color("#d48cff"), ""))
...
func make_unit(
	...
	return {
		...
		"focus": 2,
		"cooldowns": {},
		"secondary": secondary_skill_for_id(id),
		"boss_phase": 1
	}
...
func skill_name(skill_id: String) -> String:
	match skill_id:
		"hat":
			return "Coup de chapeau"
		"heal":
			return "Sauvetage élastique"
		"funk":
			return "Spore Funk"
		"flare":
			return "Tir prismatique"
		"taunt":
			return "Provocation myco"
		"mist":
			return "Brume tonique"
		"corrode":
			return "Basse corrosive"
		"mark":
			return "Balise prismatique"
		_:
			return "Action"


func skill_cost(skill_id: String) -> int:
	return 2 if skill_id == "mist" else 1


func skill_cooldown(skill_id: String) -> int:
	return 3 if skill_id in ["funk", "mist", "mark"] else 2
...
func can_use_skill(unit: Dictionary, skill_id: String) -> bool:
	return not skill_id.is_empty() and int(unit.get("focus", 0)) >= skill_cost(skill_id) and cooldown_left(unit, skill_id) <= 0 and not bool(unit["has_acted"])


func spend_skill(unit: Dictionary, skill_id: String) -> void:
	unit["focus"] = max(0, int(unit.get("focus", 0)) - skill_cost(skill_id))
	var cooldowns: Dictionary = unit.get("cooldowns", {})
	cooldowns[skill_id] = skill_cooldown(skill_id)
	unit["cooldowns"] = cooldowns
	unit["has_acted"] = true


func tick_round_resources(unit: Dictionary) -> void:
	unit["focus"] = min(int(unit.get("max_focus", 2)), int(unit.get("focus", 0)) + 1)
	var cooldowns: Dictionary = unit.get("cooldowns", {})
...
func _on_special_pressed() -> void:
	if game_over or enemy_phase or campaign_mode != "battle":
		return
	var unit = selected_unit()
	if unit.is_empty() or unit["id"] != active_unit_id or unit["has_acted"]:
		return
	var skill_id = str(unit["special"])
	if not can_use_skill(unit, skill_id):
		log_message("%s indisponible : Focus %d/%d • cooldown %d." % [skill_name(skill_id), int(unit["focus"]), skill_cost(skill_id), cooldown_left(unit, skill_id)])
		update_ui()
		return
	...
	match skill_id:
		...
		"heal":
			...
			if ally.is_empty():
				...
				update_ui()
				return
			var heal_power = 5 if int(hero_modules.get("pipo", 0)) == 0 else 4
			var healed = min(heal_power, int(ally["max_hp"]) - int(ally["hp"]))
			...
			if int(hero_modules.get("pipo", 0)) == 1:
				add_status(unit, "guarded")
			spend_skill(unit, skill_id)
		...
		"funk":
			...
			return
		"flare":
			var prism_target = best_special_target(unit)
			if prism_target.is_empty():
				log_message("TIR PRISMATIQUE : aucune cible visible à portée.")
				update_ui()
				return
			unit["facing"] = direction_from_to(unit["pos"], prism_target["pos"])
			var prism_damage = int(unit["attack"]) + 1
			spawn_damage_feedback(unit, prism_target, prism_damage, SKY)
			apply_damage(prism_target, prism_damage)
			spend_skill(unit, skill_id)
			log_message("Luma déclenche TIR PRISMATIQUE : %d dégâts, couverture ignorée." % prism_damage)
		_:
			return
...
func _on_secondary_pressed() -> void:
	...
	if unit.is_empty() or unit["id"] != active_unit_id or bool(unit["has_acted"]):
		return
	var skill_id = str(unit.get("secondary", ""))
	if not can_use_skill(unit, skill_id):
		log_message("%s indisponible : Focus %d/%d • cooldown %d." % [skill_name(skill_id), int(unit["focus"]), skill_cost(skill_id), cooldown_left(unit, skill_id)])
		update_ui()
		return
	match skill_id:
		"taunt":
			...
			for target in units:
				if target["team"] == "enemy" and int(target["hp"]) > 0 and manhattan(unit["pos"], target["pos"]) <= 1:
					add_status(target, "slowed")
			spend_skill(unit, skill_id)
			log_message("Momo lance PROVOCATION MYCO : GARDE, réaction rechargée et voisins ralentis.")
		"mist":
			var healed_count = 0
			for ally in units:
				if ally["team"] == "player" and int(ally["hp"]) > 0 and manhattan(unit["pos"], ally["pos"]) <= 2:
					var healed = min(2, int(ally["max_hp"]) - int(ally["hp"]))
					ally["hp"] = int(ally["hp"]) + healed
					add_status(ally, "guarded")
					if healed > 0:
						spawn_floating_text(ally["pos"], "+%d" % healed, MINT)
						healed_count += 1
			spend_skill(unit, skill_id)
			log_message("Pipo diffuse BRUME TONIQUE : soin de zone + GARDE (%d soigné(s))." % healed_count)
		"corrode":
			var target = best_special_target(unit)
			if target.is_empty():
				log_message("BASSE CORROSIVE : aucune cible visible.")
				return
			spawn_damage_feedback(unit, target, 1, VIOLET)
			apply_damage(target, 1)
			if int(target["hp"]) > 0:
				add_status(target, "weakened")
			spend_skill(unit, skill_id)
			log_message("Ziggy joue BASSE CORROSIVE : 1 dégât et AFFAIBLI (-1 ATQ).")
		"mark":
			var target = best_special_target(unit)
			if target.is_empty():
				log_message("BALISE PRISMATIQUE : aucune cible visible.")
				return
			add_status(target, "marked")
			spawn_particles(target["pos"], SKY, 8)
			spend_skill(unit, skill_id)
			log_message("Luma pose une BALISE PRISMATIQUE sur %s : prochain dégât +1." % target["name"])
		_:
			return
	check_mission_end()
	refresh_all()
...
func best_special_target(source: Dictionary) -> Dictionary:
	...
	for target in units:
		if target["team"] != "enemy" or int(target["hp"]) <= 0:
			continue
		if manhattan(source["pos"], target["pos"]) > effective_range(source):
			continue
		if int(source["range"]) > 1 and not has_line_of_sight(source["pos"], target["pos"], source["id"], target["id"]):
			continue
		var score = int(target["hp"]) * 10 + manhattan(source["pos"], target["pos"])
		if score < best_score:
			best_score = score
			best = target
	return best
...
func execute_spore_funk(unit: Dictionary, center: Vector2i) -> void:
	...
	spend_skill(unit, "funk")
	special_targeting = false
	special_target_cells.clear()
	var funk_damage = 3 if int(hero_modules.get("ziggy", 0)) == 0 else 2
	if targets.is_empty():
		log_message("Ziggy lâche SPORE FUNK dans le vide. C'était au moins très photogénique.")
	else:
		log_message("Ziggy lâche SPORE FUNK : %d cible(s), %d dégâts, RALENTI et onde de choc." % [targets.size(), funk_damage])
	for target in targets:
		spawn_impact_feedback(target, funk_damage, VIOLET, true)
		apply_damage(target, funk_damage)
		if int(target["hp"]) > 0:
			add_status(target, "slowed")
...
func run_enemy_activation(enemy: Dictionary) -> void:
	...
	if not used_enemy_skill and not target.is_empty():
		...
		apply_enemy_attack_trait(enemy, target)
		if int(profile["guard_penalty"]) > 0:
			remove_status(target, "guarded")
		log_message(
			"%s attaque %s : -%d PV%s."
	...
	match str(enemy["id"]):
		...
		"choriste":
			...
			if not ally.is_empty() and manhattan(enemy["pos"], ally["pos"]) <= 3:
				var heal = min(1, int(ally["max_hp"]) - int(ally["hp"]))
				if heal > 0:
...
func try_enemy_skill(enemy: Dictionary) -> bool:
	if int(enemy["hp"]) <= 0:
		return false
	if enemy["id"] == "dj_morille" and enemy_skill_ready(enemy, "bass_drop"):
		var target = closest_player_to(enemy["pos"], 2)
		if not target.is_empty():
			spawn_impact_feedback(target, 2, CORAL, true)
			apply_damage(target, 2)
			if int(target["hp"]) > 0:
				add_status(target, "slowed")
			spend_enemy_skill(enemy, "bass_drop")
			log_message("DJ MORILLE — BASS DROP : %s subit 2 dégâts et RALENTI." % target["name"])
			return true
	if enemy["id"] == "archiviste" and int(enemy.get("boss_phase", 1)) >= 2 and enemy_skill_ready(enemy, "archive_pulse"):
		var victims: Array = []
		for player in units:
		...
		if not victims.is_empty():
			...
			for victim in victims:
				spawn_impact_feedback(victim, pulse_damage, VIOLET, true)
				apply_damage(victim, pulse_damage)
...
func finish_mission_result(victory: bool, message: String) -> void:
	...
	log_message(message)
	refresh_all()
	var serial = battle_serial
	var timer = get_tree().create_timer(0.75)
	timer.timeout.connect(_open_debrief_if_current.bind(serial))


func _open_debrief_if_current(serial: int) -> void:
	if serial != battle_serial or campaign_mode != "battle" or not game_over:
		return
	show_debrief_screen()
...
func resolve_end_of_activation(unit: Dictionary) -> void:
	...
	if has_status(unit, "weakened"):
		remove_status(unit, "weakened")
...
func advance_activation() -> void:
	...
	if timeline_index >= timeline_order.size():
		if mission_objective == "survive" and current_turn >= survival_rounds:
...
func update_ui() -> void:
	...
	if unit.is_empty():
		...
		selected_label.text = (
			"%s — %s\nPV %d/%d • Focus %d/%d • MVT %d • Portée %d%s • INIT %d\n%s • face %s • %s / %s%s"
			% [
				unit["name"], unit["role"], int(unit["hp"]), int(unit["max_hp"]), int(unit["focus"]), int(unit["max_focus"]), effective_move(unit),
		...
		special_button.text = "%s • F%d • CD %s" % [skill_name(primary_id), skill_cost(primary_id), ("prêt" if primary_cd <= 0 else str(primary_cd))]
		secondary_button.text = "%s • F%d • CD %s" % [skill_name(secondary_id), skill_cost(secondary_id), ("prêt" if secondary_cd <= 0 else str(secondary_cd))]
		if special_targeting:
		...
		if special_target_cells.has(hovered_cell):
			...
			for candidate in units:
				if candidate["team"] == "enemy" and int(candidate["hp"]) > 0 and manhattan(hovered_cell, candidate["pos"]) <= 1:
					victims += 1
			var funk_preview_damage = 3 if int(hero_modules.get("ziggy", 0)) == 0 else 2
...
func status_text(unit: Dictionary) -> String:
	...
	if has_status(unit, "weakened"):
		labels.append("AFFAIBLI")
	if has_status(unit, "marked"):
		labels.append("BALISÉ")
...
func damage_profile(
	...
	var weakened_penalty = 1 if has_status(attacker, "weakened") else 0
	var marked_bonus = 1 if has_status(target, "marked") else 0
	var damage = max(1, base_damage + height_bonus + back_bonus + side_bonus + marked_bonus - cover_penalty - guard_penalty - weakened_penalty)
	var summary = ""
	var details = ""
	if height_bonus > 0:
		summary += " (+1 hauteur)"
...
func apply_damage(target: Dictionary, amount: int) -> void:
	...
	if amount > 0 and has_status(target, "marked"):
		remove_status(target, "marked")
	if target["id"] == "archiviste" and int(target["hp"]) > 0:
...
func draw_unit(unit: Dictionary) -> void:
	...
	if has_status(unit, "weakened"):
		draw_circle(center + Vector2(-24.0, -12.0), 7.0, CORAL)
		draw_string(ThemeDB.fallback_font, center + Vector2(-28.0, -8.0), "A", HORIZONTAL_ALIGNMENT_LEFT, -1.0, 9, BG)
	if has_status(unit, "marked"):
		draw_arc(center, 32.0, 0.0, TAU, 24, SKY, 2.0, true)
